...
cohort_snp$OUTCOME = cohort_snp[,OUTCOME]-1 # recode from plink coding to 0, 1
# ANALYSIS
test.listfunc = function(x){
  # Models
  MODEL = paste0("OUTCOME~" , "`", SNPs[x], "` + ", COVPC)
  glm(eval(parse(text = MODEL)), data = cohort_snp, family=binomial())
  testModel = try(glm(eval(parse(text = MODEL)), data = cohort_snp, family=binomial()),silent = T)
  if(class(testModel)[1]=="try-error"){
    sumstat=c(SNPs[x], "NoConverge", rep(NA,5), MODEL)
  }else{
    ...
    if(grep(v_interest, rownames(temp)) %>% length == 0){ # In this case, SNP is dropeed from the model
      ...
      sumstat <- c(SNPs[x], RES[3], RES[1], RES[2], RES[4], length(testModel$y), sum(testModel$y), mean(s[,SNPs[x]])/2)
    }
  }
  return(sumstat)
}
...
names(temp2)=c("POS_A2_A1", "Zvalue", "BETA", "SE", "P", "N", "E", "ALT_Frq")
temp3 = temp2 %>% separate(POS_A2_A1, c("SNP", "A2", "A1"),sep="_")
dir.create(OUTPUT, recursive = T, showWarnings = F)
FILENAME2 = sub('_', '.', basename(PHENO)) %>% sub('txt', paste(MODEL, OUTCOME, 'lgs.txt', sep='.'), .)
